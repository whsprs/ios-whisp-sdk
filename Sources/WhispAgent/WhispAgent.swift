...
public final class WhispAgent {
    ...
    public func send(message: WhispAgentSendTextMessage) throws {
    ...
    public func send(transactionId: String, txHash: String) throws {
        webSocket.write(
            data: try eventEncoder.encode(
                data: WhispAgentSendTransactionTxHash(id: transactionId, txHash: txHash)
            )
        )
    }
    
    public func send(signedTransaction: WhispAgentSendSignedTransaction) throws {
    ...
    public func send(declinedTransactionId: String) throws {
        webSocket.write(
            data: try eventEncoder.encode(
                data: WhispAgentSendDeclinedTransaction(id: declinedTransactionId)
            )
        )
    }
